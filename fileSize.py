...
def get_total_file_size(file_paths):
    """获取相同子目录下的文件大小"""
    total_size = 0
    if not tools.check_is_not_None(file_paths):
...
def filter_files_by_sizeordate():
    ...
    if not tools.check_is_not_None(paths):
        paths = tools.get_file_paths_list_limit(paths, tuple(exclude_extensions))
    if not tools.check_is_not_None(folder):
